...
export default class Registration extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isVerified: false, // Add isVerified state
    };
  }
  ...
  verifyAndRegisterVoter = async () => {
    const { account, voterPhone } = this.state;
    fetch("http://localhost:3001/verify", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ account, phone: voterPhone }),
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        if (data.isVerified) {
          ...
          alert("Account address and phone number do not match");
        }
      })
      .catch((error) => {
        console.error(
          "Error verifying account address and phone number:",
          error.message
        );
        alert(
          "Error verifying account address and phone number. Please try again."
        );
      });
  };
  ...
  handleRegistrationSuccess = () => {
    // Call the function to register the voter
  ...
  render() {
    ...
    if (this.state.isVerified) {
      // Render the App component if data is verified
      return <App phoneNumber={"+91" + this.state.voterPhone} onVerificationSuccess={this.handleRegistrationSuccess} />;
    }
              ...
              <div className="container-item">
                <form>
                  ...
                  <button
                    ...
                    onClick={this.verifyAndRegisterVoter}
                  >
...
export function loadAllVoters(voters) {
  const renderAllVoters = (voter, index) => {
    return (
